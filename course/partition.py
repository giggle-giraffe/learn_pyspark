...
def add_columns(df, n):
    for i in range(n):
        df = df.withColumn(f"newCol_{i}", col("id") * (i + 1))
    return df
...
def exercise():
    ...
    table = table.repartition(10, col("id"))
    narrow_table = narrow_table.repartition(7, col("id"))
    wide_table = add_columns(table, 30)
    join2 = wide_table.join(narrow_table, "id")
    start_time = time()
    join2.show()  # 5 seconds
    print(f"Time: {time() - start_time}")
